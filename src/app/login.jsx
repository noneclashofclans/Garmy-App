...
export default function login() {
    ...
    const handleLogin = async () => {
        ...
        try {
            const response = await fetch('https://golden-back-pftn.onrender.com/api/auth/login', {
                method: 'POST',
                headers: {
            ...
            if (response.ok) {
                const userName = data.user?.name || data.user?.username || data.name || data.username || 'back';
                const userEmail = data.user?.email || data.email || trimmedEmail;
            ...
            Alert.alert('Connection Error', 'Unable to connect to the backend server.');
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <SafeAreaView style={styles.container}>
            ...
            <Text style={styles.reg}>
                New user? <Text style={{ color: '#70b56b' }} onPress={() => router.push('/register')}>Register</Text> now!
            </Text>
        </SafeAreaView>
    );
}
